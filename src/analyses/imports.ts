...
export default {
    ...
    async run(addon: CachedAddon) {
        const imports = [];
        const importCandidates = [...addon.file_content.matchAll(importRegex)].map(i => cleanUrl(i[1]));
        for (const match of importCandidates) {
            try {
                const importUrl = new URL(match);
                imports.push(importUrl.toString());

                const remoteCss = await ky.get(importUrl.toString()).text();
                const remoteCandidates = [...remoteCss.matchAll(importRegex)].map(i => cleanUrl(i[1]));

                for (const remoteMatch of remoteCandidates) {
                    try {
                        const remoteUrl = new URL(remoteMatch);
                        imports.push(remoteUrl.toString());
                    }
                    catch {
                        continue;
                    }
